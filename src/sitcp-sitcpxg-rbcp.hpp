...
public:
    ...
    std::vector<uint8_t> read(uint32_t address, size_t length) {
        if (length == 0 || length > 255) throw Error("invalid RBCP read length");
...
private:
    std::vector<uint8_t> transaction(uint8_t command, uint32_t address,
        ...
        if (fd < 0) { freeaddrinfo(result); throw Error(strerror(errno)); }
        ...
        if (ready == 0) { close(fd); throw Timeout("RBCP timeout from " + host_ + ":" + std::to_string(port_)); }
        if (ready < 0) { close(fd); throw Error(strerror(errno)); }

        uint8_t reply[263];
        const ssize_t received = recvfrom(fd, reply, sizeof(reply), 0, nullptr, nullptr);
        close(fd);
        if (received < 8 || reply[0] != 0xff || reply[2] != id) throw Error("invalid RBCP reply");
        if ((reply[1] & 0x01u) != 0) throw BusError("RBCP bus error");
