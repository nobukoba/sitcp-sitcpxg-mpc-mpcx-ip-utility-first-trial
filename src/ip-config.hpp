...
public:
    ...
    void write(uint32_t address, const std::vector<uint8_t>& data) {
        ...
        const auto reply = transaction(
            0x80, address, data, static_cast<uint8_t>(data.size()));
        if (reply.size() != data.size()) {
            throw Error("short RBCP write acknowledgement");
        }
    }
...
inline std::vector<uint8_t> read_retry(Client& client,
                                       uint32_t address,
                                       uint8_t length) {
...
inline void print_snapshot(const Snapshot& snapshot, const std::string& prefix = "") {
    std::cout << prefix << "current MAC  : " << mac(snapshot.current_mac) << '\n'
              << prefix << "current IP   : " << ipv4(snapshot.current_ip) << '\n'
              << prefix << "EEPROM MAC   : " << mac(snapshot.eeprom_mac) << '\n'
              << prefix << "EEPROM IP    : " << ipv4(snapshot.eeprom_ip) << '\n';
}

inline Snapshot show_all(const std::string& host,
                         uint16_t port,
                         double timeout,
...
inline void write_current_ip(const std::string& host,
    ...
    try {
        ...
        // The IP can change before the acknowledgement reaches the old address.
